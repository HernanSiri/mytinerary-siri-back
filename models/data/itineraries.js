...
async function createItineraries(arrayItineraries) {
    try {
        ...
        for (let itinerary of arrayItineraries) {
            let city = await City.findOne({ city:itinerary.city_id})   //busco el usuario que conincida el mail del objeto
            let city_id = await city._id                           //SOLO necesito el id de ese usuario
            itinerary.city_id = city_id                                //reasigno el valor del admin_id de cada ciudad para cambiar el mail que viene por default POR LO QUE CORRESPONDE (QUE SERIA EL ID)
