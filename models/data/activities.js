...
async function createActivities(arrayActivities) {
    try {
        ...
        for (let activity of arrayActivities) {
            let itinerary = await Itinerary.findOne({ name:activity.itinerary_id})   //busco el usuario que conincida el mail del objeto
            let itinerary_id = await itinerary._id                           //SOLO necesito el id de ese usuario
            activity.itinerary_id = itinerary_id                               //reasigno el valor del admin_id de cada ciudad para cambiar el mail que viene por default POR LO QUE CORRESPONDE (QUE SERIA EL ID)
