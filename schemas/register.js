...
let registerSchema = joi.object({
    ...
    mail: joi.string().required().min(3).email().max(30).messages({
    ...
    photo: joi.string().min(3).max(100).empty("").messages({
        'string.min': "Photo Url must have at least 3 characters please!",
        "string.max": "Photo Url must be less than 100 characters please!",
    })
})
